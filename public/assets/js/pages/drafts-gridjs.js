...
class GridDatatable {
    ...
    basicTableInit() {
        ...
        async function initializeDraftTable() {
  ...
  try {
    ...
    const response = await fetch('/dashboard/tickets/data-drafts', {
      headers: {
      ...
      new gridjs.Grid({
        columns: [
          ...
          },
          {
            ...
            width: '180px',
            formatter: (cell, row) => {
              const draftId = row.cells[1].data;
              ...
              let actionsHTML = `
                <div class="flex items-center">
                  ...
                  <a href="/dashboard/draft/drafts/${draftId}" class="text-primary me-2" title="Create Ticket">
        ...
        data: drafts.map((draft, index) => [
          index + 1,
          draft.id,
          draft.fname || draft.user?.fname || 'N/A',
          draft.phone_numbers || [],
          draft.created_at,
          '' // Actions column
        ])
    ...
    if (tableContainer) {
      tableContainer.innerHTML = `
        <div class="alert alert-danger p-4">
          ...
          <p>${error.message}</p>
          <button onclick="initializeDraftTable()" class="btn btn-sm btn-primary mt-2">
            Retry
          </button>
        </div>
      `;
...
function deleteTicket(ticketId) {
  if (confirm('Are you sure you want to delete this ticket?')) {
    ...
    .then(response => {
      if (response.ok) {
        initializeDraftTable(); // Refresh table
      } else {
